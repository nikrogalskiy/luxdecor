...
let scr_items = document.querySelectorAll('._scr-item'); // добавить класс для анимации (если нужно что бы анимация не повторялась при повторном скроле добавляем еще класс _anim-no-repeat)
	...
	if (scr_items.length > 0) {
		for (let index = 0; index < scr_items.length; index++) {
			...
			if ((src_value > scr_item_offset - scr_item_point) && src_value < (scr_item_offset + scr_item_height)) {
				...
				if (!scr_item.classList.contains('_anim-no-repeat')) {
...
let sliderPage = new Swiper('.slider-page', {
	observer: true,
	observeParents: true,
	slidesPerView: 1,
	spaceBetween: 0,
...
let sliderHit = new Swiper('.slider-hit', {
	observer: true,
	observeParents: true,
	slidesPerView: 4,
	spaceBetween: 0,
	...
	// Arrows
	navigation: {
		nextEl: '.slider-hit__arrow_next',
		prevEl: '.slider-hit__arrow_prev',
	},
	breakpoints: {
		320: {
			slidesPerView: 1,
...
let sliderStock = new Swiper('.slider-stock', {
	observer: true,
	observeParents: true,
	slidesPerView: 4,
	spaceBetween: 0,
	...
	// Arrows
	navigation: {
		nextEl: '.slider-stock__arrow_next',
		prevEl: '.slider-stock__arrow_prev',
	},
	breakpoints: {
		320: {
			slidesPerView: 1,
...
//Добавляем в html клас _swiper и data-mobile="false"
//Присваем константу (sliderSpecialPage   любое название) и переменную (sliderSpecialPageBody   любое название)
//Подставляем в код свою константу и переменную
//Ширину меняем на нужную (в даном примере 600) 
...
//===Обрезка текста===================================================================================================
// Подключить библиотеку simplebar.min.js
// Подключить этот файл
// Подключить файл стилей customscroll.scss
// Добавить class="_customscroll"
//========================================================================================================================================================
/*
& .simplebar - content - wrapper {
	padding - right: 9px;
}
& .simplebar - track.simplebar - vertical {  скрол (задавать ширину, фон и др.)
	background - color: rgba(0, 0, 0, 0.2);
	width: 4px;
}
& .simplebar - scrollbar:: before {         ползунок (задавать необходимые стили)
	left: 0;
	top: 0;
	right: 0;
	bottom: 0;
...
& .simplebar - scrollbar.simplebar - visible:: before {   при наведении отключаетм исчезновение скрола
	opacity: 1;
}
*/
//========================================================================================================================================================

document.querySelectorAll('._customscroll').forEach(el => {
	new SimpleBar(el)
});
...
if (isMobile.any()) {
	...
	if (catalogTitle) {
		catalogTitle.addEventListener("mouseenter", function (e) {
			catalogTitle.classList.add("_active");
			_slideToggle(catalogBody);
		});
		catalog.addEventListener("mouseleave", function (e) {
			catalogTitle.classList.remove("_active");
			_slideUp(catalogBody);
		});
	}
}
...
if (isMobile.any()) {
	let catalogParentLink = document.querySelectorAll('.menu-catalog__list li');

	for (let index = 0; index < catalogParentLink.length; index++) {
		const catalogParent = catalogParentLink[index];
...
//========================================================================================================================================================
let articleCompareLinks = document.querySelectorAll('.article__compare_link');

if (articleCompareLinks) {

	for (let index = 0; index < articleCompareLinks.length; index++) {
		const articleCompareLink = articleCompareLinks[index];
		articleCompareLink.addEventListener("click", function (e) {
			articleCompareLink.parentElement.classList.toggle('_active');
		});
	}
}
